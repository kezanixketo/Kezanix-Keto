...
interface AppConfig {
  ...
  protocol_pdf_url_en?: string;
  protocol_image_url?: string;
  technical_sheet_pdf_url: string;
  technical_sheet_pdf_url_en?: string;
  technical_sheet_image_url?: string;
  returns_policy_pdf_url?: string;
  returns_policy_pdf_url_en?: string;
...
function AppContent() {
  ...
  const [uploading, setUploading] = useState<'product' | 'protocol' | 'technical' | 'returns' | null>(null);
  ...
  const translations = {
    es: {
      heroSub: "Elige el plan que mejor se adapte a tus objetivos de salud y bienestar.",
      freeShipping: "Envío Gratis",
      buyNow: "Comprar Ahora",
      investment: "Inversión",
      resourcesTitle: "RECURSOS EXCLUSIVOS",
      resourcesSub: "Descarga nuestras guías gratuitas para maximizar tus resultados con Keto Prime.",
      protocolBtn: "Protocolo de Alimentación",
      techSheetBtn: "Ficha Técnica",
      checkoutTitle: "Finalizar Compra",
      step1: "Realiza tu Pago",
      step1Desc: "Para completar tu pedido, primero realiza el pago a través de nuestro enlace seguro. Una vez hecho, regresa aquí para llenar tus datos de envío.",
      payBtn: "Ir a Pagar Ahora",
      step2: "Datos de Envío",
      fullName: "Nombre Completo",
      idNumber: "Número de Cédula",
      phone: "Teléfono",
      country: "País",
      city: "Provincia / Ciudad",
      district: "Distrito / Departamento",
      address: "Dirección Exacta",
      notes: "Otras Señas / Notas",
      confirmBtn: "Confirmar y Enviar por WhatsApp",
      processing: "Procesando...",
      waDesc: "Al hacer clic, se abrirá WhatsApp para enviar tus datos directamente a nuestro equipo.",
      copyright: "© 2026 KEZANIX GROUP. Todos los derechos reservados.",
      adminAccess: "Acceso Admin",
      cancel: "Cancelar",
      enter: "Entrar",
      orderNew: "NUEVO PEDIDO - KEZANIX",
      orderProduct: "Producto",
      orderPrice: "Precio",
      orderCustomer: "DATOS DEL CLIENTE",
      orderSentFrom: "Enviado desde el sitio web de Kezanix",
      returnsPolicy: "Política de Devoluciones y Reembolsos",
      returnsPolicyTitle: "POLÍTICA DE DEVOLUCIONES Y REEMBOLSOS",
      returnsPolicyText1: "Se aceptan devoluciones dentro de los 14 días posteriores a la recepción.",
      returnsPolicyText2: "Los productos deben estar sin abrir, en su empaque original y sin signos de uso.",
      returnsPolicyText3: "Para iniciar una devolución, contacte a: amaya@kezanix.com.",
      returnsPolicyText4: "Los costos de envío no son reembolsables (excepto por error de Kezanix).",
      returnsPolicyText5: "Los reembolsos se emitirán al método de pago original dentro de los 7 a 10 días hábiles posteriores a la aprobación de la devolución.",
      viewPdf: "Ver / Descargar PDF Documento"
    },
    en: {
      heroSub: "Choose the plan that best fits your health and wellness goals.",
      freeShipping: "Free Shipping",
      buyNow: "Buy Now",
      investment: "Investment",
      resourcesTitle: "EXCLUSIVE RESOURCES",
      resourcesSub: "Download our free guides to maximize your results with Keto Prime.",
      protocolBtn: "Feeding Protocol",
      techSheetBtn: "Technical Sheet",
      checkoutTitle: "Checkout",
      step1: "Complete Your Payment",
      step1Desc: "To complete your order, first make the payment through our secure link. Once done, come back here to fill in your shipping details.",
      payBtn: "Go to Pay Now",
      step2: "Shipping Details",
      fullName: "Full Name",
      idNumber: "ID Number / SSN",
      phone: "Phone Number",
      country: "Country",
      city: "City / State",
      district: "Zip Code / Area",
      address: "Exact Address",
      notes: "Additional Notes / Landmarks",
      confirmBtn: "Confirm and Send via WhatsApp",
      processing: "Processing...",
      waDesc: "Clicking will open WhatsApp to send your details directly to our team.",
      copyright: "© 2026 KEZANIX GROUP. All rights reserved.",
      adminAccess: "Admin Access",
      cancel: "Cancel",
      enter: "Login",
      orderNew: "NEW ORDER - KEZANIX",
      orderProduct: "Product",
      orderPrice: "Price",
      orderCustomer: "CUSTOMER DATA",
      orderSentFrom: "Sent from Kezanix website",
      returnsPolicy: "Returns and Refunds Policy",
      returnsPolicyTitle: "RETURNS AND REFUNDS POLICY",
      returnsPolicyText1: "Returns are accepted within 14 days of receipt.",
      returnsPolicyText2: "Products must be unopened, in original packaging, and show no signs of use.",
      returnsPolicyText3: "To initiate a return, contact: amaya@kezanix.com.",
      returnsPolicyText4: "Shipping costs are non-refundable (except when due to Kezanix’s error).",
      returnsPolicyText5: "Refunds will be issued to the original payment method within 7 to 10 business days after return approval.",
      viewPdf: "View / Download PDF Document"
    }
  };

  const isEnglish = detectedCountry === 'US';
  ...
  useEffect(() => {
    ...
    const fetchConfig = async () => {
      ...
      if (error && error.code === 'PGRST116') { // Record not found
        const defaultConfig = {
          ...
          hero_title_en: "TRANSFORM YOUR BODY WITH KETO PRIME",
          info_title: "¿POR QUÉ ELEGIR KETO PRIME?",
          info_title_en: "WHY CHOOSE KETO PRIME?",
          ...
          info_text_en: "Our advanced formula is designed to help you reach ketosis faster, burn fat as fuel, and maintain consistent energy levels throughout the day.",
    ...
    const detectCountry = async () => {
      // PERMITIR PRUEBA: Si la URL tiene ?lang=en, forzamos inglés para que el usuario pueda verificarlo
      ...
      if (urlLang === 'en') {
        ...
        setDetectedCountry('CR');
        localStorage.setItem('user_country', 'CR');
      }
      ...
      try {
        ...
        // Fallback: Si falla la IP, usar el idioma del navegador solo para la detección inicial si no hay otra
  ...
  const handleFileUpload = async (file: File, path: string, type: 'product' | 'protocol' | 'technical' | 'header' | 'returns'): Promise<string | null> => {
    console.log(`[Upload] Iniciando: ${file.name} (${(file.size/1024).toFixed(1)}KB)`);
    setUploading(type);
    try {
      const maxSize = 50 * 1024 * 1024; // Aumentado a 50MB
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('column') || error.code === '42703') {
          // Guardar estado local para que funcione en la sesión
          setConfig(newConfig);
          alert('⚠️ ATENCIÓN: Tu tabla "config" en Supabase aún no tiene las columnas para las políticas de devolución.\n\nLos cambios ya están activos en esta sesión. Para que se guarden permanentemente en tu base de datos Supabase, ve al "SQL Editor" de Supabase y ejecuta esta línea:\n\nALTER TABLE config ADD COLUMN IF NOT EXISTS returns_policy_pdf_url text, ADD COLUMN IF NOT EXISTS returns_policy_pdf_url_en text;');
          return;
        } else {
          throw error;
        }
  ...
  const handleProductsReorder = async (newOrder: Product[]) => {
    setProducts(newOrder);
    
    // Preparar las actualizaciones masivas
    const updates = newOrder.map((p, index) => ({
      ...
      sales_url: p.sales_url
    }));

    try {
      // En Supabase, usamos upsert para actualizar múltiples filas por ID
      const { error } = await supabase
        .from('products')
        .upsert(updates, { onConflict: 'id' });
        
      if (error) throw error;
      console.log('✅ Orden actualizado en BD');
    } catch (e) {
  ...
  const seedData = async () => {
    const initialProducts = [
      { 
        name: "Pack Keto Intermedio (2 Botellas)", 
        name_en: "Keto Intermediate Pack (2 Bottles)",
        price: 89.99, 
        description: "Ahorra más con nuestro paquete de 2 botellas. Perfecto para un tratamiento de 60 días.", 
        description_en: "Save more with our 2-bottle pack. Perfect for a 60-day treatment.",
        image_url: "https://picsum.photos/seed/keto2/800/800", 
        sales_url: "https://checkout.example.com/pack-2", 
        order_index: 0 
      },
      { 
        name: "Pack Inicio Keto (1 Botella)", 
        name_en: "Keto Starter Pack (1 Bottle)",
        price: 49.99, 
        description: "Ideal para comenzar tu viaje keto con los suplementos esenciales.", 
        description_en: "Ideal to start your keto journey with essential supplements.",
        image_url: "https://picsum.photos/seed/keto1/800/800", 
        sales_url: "https://checkout.example.com/pack-1", 
        order_index: 1 
      },
      { 
        name: "Pack Keto Elite (3 Botellas)", 
        name_en: "Keto Elite Pack (3 Bottles)",
        price: 129.99, 
        description: "Máximo ahorro y resultados a largo plazo. Tratamiento completo de 90 días.", 
        description_en: "Maximum savings and long-term results. Full 90-day treatment.",
        image_url: "https://picsum.photos/seed/keto3/800/800", 
        sales_url: "https://checkout.example.com/pack-3", 
        order_index: 2 
      },
    ];
          ...
          <div className="flex items-center gap-4">
            {isAdminMode && (
              <div className="flex items-center gap-2 px-3 py-1 bg-white/10 rounded-full border border-white/20">
                <span className="text-[10px] font-bold text-white uppercase opacity-70">Preview:</span>
                <button 
                  onClick={() => {
                    const next = detectedCountry === 'US' ? 'CR' : 'US';
                    setDetectedCountry(next);
                    localStorage.setItem('user_country', next);
                  }}
                  className={`px-2 py-0.5 rounded text-[10px] font-bold transition-all ${isEnglish ? 'bg-brand-lime text-brand-blue' : 'bg-brand-blue text-white ring-1 ring-white/30'}`}
                >
                  {isEnglish ? 'USA (EN)' : 'LATAM (ES)'}
                </button>
              </div>
            )}
            {isAdminMode && (
              <button 
                ...
                className="p-2 rounded-full bg-brand-blue text-white transition-colors hover:bg-brand-blue/80"
        ...
        <main className="pt-20">
          ...
          <section className="relative h-[300px] md:h-[450px] flex items-center justify-center overflow-hidden mb-12">
            ...
            <div className="relative z-10 text-center px-6 max-w-5xl">
              ...
              >
                {isEnglish ? (config?.hero_title_en || "TRANSFORM YOUR BODY WITH KETO PRIME") : config?.hero_title}
              </motion.h1>
              <p className="text-lg md:text-xl text-white/90 max-w-2xl mx-auto font-medium drop-shadow-lg">
                {isEnglish 
                  ? (config?.info_text_en ? (config.info_text_en.length > 100 ? config.info_text_en.substring(0, 100) + "..." : config.info_text_en) : "Experience the power of advanced ketosis for maximum performance and fat loss.")
                  : t.heroSub}
              </p>
            </div>
          </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {sortedProducts.map((product, idx) => (
                ...
                >
                  ...
                  <h3 className="text-2xl text-white mb-2">{(isEnglish && product.name_en) ? product.name_en : (isEnglish ? "Keto Prime Pack" : product.name)}</h3>
                  <p className="text-white/70 text-sm mb-6 line-clamp-2">
                    {isEnglish 
                      ? (product.description_en || "Advanced formula to accelerate ketosis and fat burning.") 
                      : product.description}
                  </p>
                    ...
                    <div className="flex flex-col gap-4 pt-6 border-t border-white/10">
                      <div className="flex items-end justify-between">
                        <div className="flex flex-col">
                          <span className="text-[10px] font-bold uppercase tracking-wider text-white/40 mb-1">{t.investment}</span>
            ...
            <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
              ...
              >
                <h2 className="text-4xl md:text-5xl mb-8 leading-tight">
                  {isEnglish ? (config?.info_title_en || "WHY CHOOSE KETO PRIME?") : config?.info_title}
                </h2>
                <p className="text-lg text-white/80 mb-10 leading-relaxed">
                  {isEnglish ? (config?.info_text_en || "Our advanced formula is designed to help you reach ketosis faster and maintain energy levels.") : config?.info_text}
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {(isEnglish 
                    ? (config?.benefits_en && config.benefits_en.length > 0 ? config.benefits_en : ["Accelerated fat burning", "Mental clarity & focus", "Natural appetite control", "Sustained energy"])
                    : (config?.benefits || [])
                  ).map((benefit, i) => (
          ...
          <section className="py-24 px-6 max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row gap-12 items-center">
              <div className="flex-1">
                <h2 className="text-4xl text-brand-blue mb-6">{t.resourcesTitle}</h2>
                <p className="text-brand-dark/60 mb-8">{t.resourcesSub}</p>
                <div className="flex flex-wrap gap-4">
                  <a 
                    href={isEnglish && config?.protocol_pdf_url_en ? config.protocol_pdf_url_en : config?.protocol_pdf_url} 
                    target="_blank" 
                  ...
                  >
                    <Download size={20} /> {t.protocolBtn}
                  </a>
                  <a 
                    href={isEnglish && config?.technical_sheet_pdf_url_en ? config.technical_sheet_pdf_url_en : config?.technical_sheet_pdf_url} 
                    target="_blank" 
                    rel="noopener noreferrer"
                  ...
                  >
                    <Download size={20} /> {t.techSheetBtn}
                  </a>
                </div>
              </div>
        ...
        {showAdminLogin && (
          ...
          >
            <motion.div 
              ...
              animate={{ scale: 1, y: 0 }}
              className="bg-white p-8 rounded-3xl shadow-2xl max-sm w-full"
            >
              <h3 className="text-2xl text-brand-dark mb-4">{t.adminAccess}</h3>
              <input 
                type="password"
                value={adminPass}
        ...
        {showReturnsModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6">
            ...
            <motion.div 
              ...
              className="relative bg-white w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-[2.5rem] shadow-2xl p-6 md:p-8 text-brand-dark flex flex-col"
            >
              <div className="flex justify-between items-start border-b border-brand-blue/10 pb-4 mb-4 shrink-0">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-brand-blue/10 text-brand-blue rounded-full flex items-center justify-center">
                    <FileText size={20} />
                  </div>
                  <div>
                    <h2 className="text-lg md:text-xl font-archivo text-brand-blue font-bold tracking-tight">
                      {t.returnsPolicyTitle}
                    </h2>
                    <p className="text-xs text-brand-dark/40 font-medium mt-0.5">KEZANIX GROUP</p>
                  </div>
                </div>
                <button 
                  onClick={() => setShowReturnsModal(false)}
                  className="w-9 h-9 rounded-full bg-brand-blue/5 flex items-center justify-center text-brand-blue hover:bg-brand-blue hover:text-white transition-all shrink-0"
                >
                  <X size={18} />
                </button>
              </div>

              {(isEnglish ? (config?.returns_policy_pdf_url_en || config?.returns_policy_pdf_url) : (config?.returns_policy_pdf_url || config?.returns_policy_pdf_url_en)) ? (
                <div className="space-y-4">
                  <div className="w-full h-[60vh] rounded-2xl overflow-hidden border border-brand-blue/10 bg-gray-50 shadow-inner">
                    <iframe 
                      src={isEnglish ? (config?.returns_policy_pdf_url_en || config?.returns_policy_pdf_url) : (config?.returns_policy_pdf_url || config?.returns_policy_pdf_url_en)} 
                      className="w-full h-full border-none"
                      title="Returns and Refunds Policy PDF"
                    />
                  </div>
                  <a
                    href={isEnglish ? (config?.returns_policy_pdf_url_en || config?.returns_policy_pdf_url) : (config?.returns_policy_pdf_url || config?.returns_policy_pdf_url_en)}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full bg-brand-blue text-white py-3.5 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-brand-light transition-all shadow-md"
                  >
                ...
                <div className="space-y-4 text-xs md:text-sm font-medium leading-relaxed text-brand-dark/80 bg-brand-blue/5 p-6 rounded-2xl border border-brand-blue/10">
                  <p className="flex items-start gap-2">
                    <span className="text-brand-blue font-bold">•</span>
                    <span>{t.returnsPolicyText1}</span>
                  </p>
                  <p className="flex items-start gap-2">
                    <span className="text-brand-blue font-bold">•</span>
                    <span>{t.returnsPolicyText2}</span>
                  </p>
                  <p className="flex items-start gap-2">
                    <span className="text-brand-blue font-bold">•</span>
                    <span>
                      To initiate a return, contact:{' '}
                      <a href="mailto:amaya@kezanix.com" className="text-brand-blue font-bold underline">
                        amaya@kezanix.com
                      </a>
                    </span>
                  </p>
                  <p className="flex items-start gap-2">
                    <span className="text-brand-blue font-bold">•</span>
                    <span>{t.returnsPolicyText4}</span>
                  </p>
                  <p className="flex items-start gap-2">
                    <span className="text-brand-blue font-bold">•</span>
                    <span>{t.returnsPolicyText5}</span>
                  </p>
                </div>
              )}

              <div className="mt-6 pt-4 border-t border-brand-blue/5 flex justify-between items-center text-[10px] text-brand-dark/40 font-bold uppercase tracking-wider shrink-0">
                <span>KEZANIX GROUP</span>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-6 order-2 md:order-1 text-center sm:text-left">
            ...
            {(() => {
              const pdfUrl = isEnglish 
                ? (config?.returns_policy_pdf_url_en || config?.returns_policy_pdf_url) 
                : (config?.returns_policy_pdf_url || config?.returns_policy_pdf_url_en);
              ...
              if (pdfUrl) {
                return (
                  <a 
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-white/80 hover:text-white text-xs font-bold underline underline-offset-4 transition-colors flex items-center gap-1.5"
                  >
                    <FileText size={14} />
                    {t.returnsPolicy}
                  </a>
...
interface AdminPanelProps {
  ...
  onFileUpload: (file: File, path: string, type: 'product' | 'protocol' | 'technical' | 'header' | 'returns') => Promise<string | null>;
  uploading: 'product' | 'protocol' | 'technical' | 'header' | 'returns' | null;
  onProductsReorder: (newOrder: Product[]) => void;
}
    ...
    <div className="pt-24 pb-20 px-6 max-w-5xl mx-auto space-y-16">
      ...
      <section className="bg-brand-blue/5 p-8 rounded-3xl space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4 p-4 bg-white/50 rounded-2xl border border-brand-blue/10">
            <h4 className="text-xs font-bold text-brand-blue uppercase">Contenido Principal (Español)</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase text-brand-dark/40">Título Hero</label>
                <input 
                  type="text" 
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase text-brand-dark/40">Título Info</label>
                <input 
                  type="text" 
              ...
              <div className="md:col-span-2 space-y-2">
                <label className="text-[10px] font-bold uppercase text-brand-dark/40">Texto Informativo</label>
                <textarea 
                  rows={3}
          ...
          <div className="space-y-4 p-4 bg-brand-blue/5 rounded-2xl border border-brand-blue/10">
            <h4 className="text-xs font-bold text-brand-blue uppercase">Main Content (English)</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase text-brand-dark/40">Hero Title (EN)</label>
                <input 
                  type="text" 
                  placeholder="Focusing on the US market"
                  value={localConfig?.hero_title_en || ''} 
                  onChange={e => setLocalConfig(prev => prev ? { ...prev, hero_title_en: e.target.value } : null)}
                  className="w-full p-3 rounded-xl border border-brand-blue/10 focus:ring-2 ring-brand-blue outline-none bg-white"
                />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase text-brand-dark/40">Info Title (EN)</label>
                <input 
                  type="text" 
                  placeholder="Why choose Keto Prime?"
                  value={localConfig?.info_title_en || ''} 
                  onChange={e => setLocalConfig(prev => prev ? { ...prev, info_title_en: e.target.value } : null)}
                  className="w-full p-3 rounded-xl border border-brand-blue/10 focus:ring-2 ring-brand-blue outline-none bg-white"
                />
              </div>
              <div className="md:col-span-2 space-y-2">
                <label className="text-[10px] font-bold uppercase text-brand-dark/40">Info Text (EN)</label>
                <textarea 
                  rows={3}
                  placeholder="Detailed description for English speakers"
                  value={localConfig?.info_text_en || ''} 
                  onChange={e => setLocalConfig(prev => prev ? { ...prev, info_text_en: e.target.value } : null)}
                  className="w-full p-3 rounded-xl border border-brand-blue/10 focus:ring-2 ring-brand-blue outline-none bg-white"
                />
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              <label className="text-xs font-bold uppercase text-brand-dark/40">Beneficios (Español)</label>
              {(localConfig?.benefits || []).map((benefit, idx) => (
                <div key={idx} className="flex gap-2">
                  <input 
                    type="text"
                    value={benefit}
                    onChange={(e) => {
                      const newBenefits = [...(localConfig?.benefits || [])];
                      newBenefits[idx] = e.target.value;
                      setLocalConfig(prev => prev ? { ...prev, benefits: newBenefits } : null);
                    }}
                    className="flex-1 p-2 rounded-lg border border-brand-blue/10 text-sm outline-none"
                  />
                  <button 
                    onClick={() => {
                      const newBenefits = (localConfig?.benefits || []).filter((_, i) => i !== idx);
                      setLocalConfig(prev => prev ? { ...prev, benefits: newBenefits } : null);
                    }}
              ...
              <button 
                onClick={() => setLocalConfig(prev => prev ? { ...prev, benefits: [...(prev.benefits || []), ''] } : null)}
                className="w-full py-2 border-2 border-dashed border-brand-blue/20 rounded-lg text-brand-blue/40 text-xs font-bold hover:border-brand-blue/40 hover:text-brand-blue transition-all"
              >
                + Añadir Beneficio
              </button>
            </div>
            <div className="space-y-4">
              <label className="text-xs font-bold uppercase text-brand-dark/40">Benefits (English)</label>
              {(localConfig?.benefits_en || []).map((benefit, idx) => (
                <div key={idx} className="flex gap-2">
                  <input 
                    type="text"
                    value={benefit}
                    ...
                    onChange={(e) => {
                      const newBenefits = [...(localConfig?.benefits_en || [])];
                      newBenefits[idx] = e.target.value;
                      setLocalConfig(prev => prev ? { ...prev, benefits_en: newBenefits } : null);
                    }}
                    className="flex-1 p-2 rounded-lg border border-brand-blue/10 text-sm outline-none"
                  />
                  <button 
                    onClick={() => {
                      const newBenefits = (localConfig?.benefits_en || []).filter((_, i) => i !== idx);
                      setLocalConfig(prev => prev ? { ...prev, benefits_en: newBenefits } : null);
                    }}
                    className="p-2 text-red-500 hover:bg-red-50 rounded-lg"
                  >
              ...
              <button 
                onClick={() => setLocalConfig(prev => prev ? { ...prev, benefits_en: [...(prev.benefits_en || []), ''] } : null)}
                className="w-full py-2 border-2 border-dashed border-brand-blue/20 rounded-lg text-brand-blue/40 text-xs font-bold hover:border-brand-blue/40 hover:text-brand-blue transition-all"
              >
                + Add Benefit (EN)
              </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
            <div className="space-y-2">
              <label className="text-[10px] font-bold uppercase text-brand-dark/40">URL Video (YouTube o Vimeo)</label>
              <input 
                type="text" 
          ...
          <div className="md:col-span-2 space-y-8 mt-6">
            <div className="p-6 bg-white/50 rounded-3xl border border-brand-blue/10 shadow-sm">
              <h4 className="text-xs font-bold text-brand-blue uppercase tracking-widest mb-6 flex items-center gap-2">
                <FileText size={16} /> Documentación (Español)
              </h4>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div className="space-y-3">
                  <label className="text-[10px] font-bold uppercase text-brand-dark/40 flex items-center gap-1">
                    Protocolo PDF (ES) <span className="text-brand-blue/20">• Obligatorio</span>
                  </label>
                  <div className="flex gap-2">
                    <input 
                      ...
                      className="flex-1 p-3 rounded-xl border border-brand-blue/10 bg-white/80 outline-none text-xs truncate"
                    />
                    <label className="bg-brand-blue text-white p-3 rounded-xl cursor-pointer hover:bg-brand-light transition-all flex items-center justify-center min-w-[50px] shadow-lg shadow-brand-blue/20">
                      {uploading === 'protocol' ? (
                        <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: "linear" }} className="w-5 h-5 border-2 border-white border-t-transparent rounded-full" />
                      ) : <Upload size={20} />}
                      <input 
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-bold uppercase text-brand-dark/40 flex items-center gap-1">
                    Ficha Técnica PDF (ES) <span className="text-brand-blue/20">• Obligatorio</span>
                  </label>
                  <div className="flex gap-2">
                    <input 
                      type="text" 
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-bold uppercase text-brand-dark/40 flex items-center gap-1">
                    Política Devoluciones PDF (ES)
                  </label>
                  <div className="flex gap-2">
                    <input 
                      type="text" 
                      value={localConfig?.returns_policy_pdf_url || ''} 
                      readOnly
                      placeholder="Sin archivo..."
                      className="flex-1 p-3 rounded-xl border border-brand-blue/10 bg-white/80 outline-none text-xs truncate"
                    />
                    <label className="bg-brand-blue text-white p-3 rounded-xl cursor-pointer hover:bg-brand-light transition-all flex items-center justify-center min-w-[50px] shadow-lg shadow-brand-blue/20">
                      ...
                      <input 
                        ...
                        onChange={async (e) => {
                          const file = e.target.files?.[0];
                          if (file) {
                            const url = await onFileUpload(file, `resources/returns_${Date.now()}.pdf`, 'returns');
                            if (url) setLocalConfig(prev => prev ? { ...prev, returns_policy_pdf_url: url } : null);
                          }
                        }}
                      />
                    </label>
            ...
            <div className="p-6 bg-brand-blue/5 rounded-3xl border-2 border-brand-blue/20 shadow-inner">
              <h4 className="text-xs font-bold text-brand-blue uppercase tracking-widest mb-6 flex items-center gap-2">
                <Globe size={16} /> Documentation (English - USA)
              </h4>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div className="space-y-3">
                  <label className="text-[10px] font-bold uppercase text-brand-blue/40">Protocol PDF (EN)</label>
                  <div className="flex gap-2">
                    <input 
                      type="text" 
                      value={localConfig?.protocol_pdf_url_en || ''} 
                      readOnly
                      placeholder="Upload English PDF..."
                      className="flex-1 p-3 rounded-xl border border-brand-blue/10 bg-white outline-none text-xs truncate"
                    />
                    <label className="bg-brand-blue text-white p-3 rounded-xl cursor-pointer hover:bg-brand-light transition-all flex items-center justify-center min-w-[50px] shadow-lg shadow-brand-blue/10">
                      {uploading === 'protocol' ? (
                        <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: "linear" }} className="w-5 h-5 border-2 border-white border-t-transparent rounded-full" />
                      ) : <Upload size={20} />}
                      <input 
                        ...
                        onChange={async (e) => {
                          const file = e.target.files?.[0];
                          if (file) {
                            const url = await onFileUpload(file, `resources/protocol_en_${Date.now()}.pdf`, 'protocol');
                            if (url) setLocalConfig(prev => prev ? { ...prev, protocol_pdf_url_en: url } : null);
                          }
                        }}
                      />
                    </label>
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-bold uppercase text-brand-blue/40">Technical Sheet PDF (EN)</label>
                  <div className="flex gap-2">
                    <input 
                      type="text" 
                      value={localConfig?.technical_sheet_pdf_url_en || ''} 
                      readOnly
                      placeholder="Upload English PDF..."
                      className="flex-1 p-3 rounded-xl border border-brand-blue/10 bg-white outline-none text-xs truncate"
                    />
                    <label className="bg-brand-blue text-white p-3 rounded-xl cursor-pointer hover:bg-brand-light transition-all flex items-center justify-center min-w-[50px] shadow-lg shadow-brand-blue/10">
                      ...
                      <input 
                        ...
                        onChange={async (e) => {
                          const file = e.target.files?.[0];
                          if (file) {
                            const url = await onFileUpload(file, `resources/technical_en_${Date.now()}.pdf`, 'technical');
                            if (url) setLocalConfig(prev => prev ? { ...prev, technical_sheet_pdf_url_en: url } : null);
                          }
                        }}
                      />
                    </label>
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-bold uppercase text-brand-blue/40">Returns Policy PDF (EN)</label>
                  <div className="flex gap-2">
                    <input 
                      type="text" 
                      value={localConfig?.returns_policy_pdf_url_en || ''} 
                      readOnly
                      placeholder="Upload English PDF..."
                      className="flex-1 p-3 rounded-xl border border-brand-blue/10 bg-white outline-none text-xs truncate"
                    />
                    <label className="bg-brand-blue text-white p-3 rounded-xl cursor-pointer hover:bg-brand-light transition-all flex items-center justify-center min-w-[50px] shadow-lg shadow-brand-blue/10">
                      ...
                      <input 
                        ...
                        onChange={async (e) => {
                          const file = e.target.files?.[0];
                          if (file) {
                            const url = await onFileUpload(file, `resources/returns_en_${Date.now()}.pdf`, 'returns');
                            if (url) setLocalConfig(prev => prev ? { ...prev, returns_policy_pdf_url_en: url } : null);
                          }
                        }}
                      />
                    </label>
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button 
              onClick={seedData}
              className="flex items-center gap-2 bg-brand-blue/5 text-brand-blue px-4 py-2 rounded-xl font-bold text-sm"
              title="Cargar productos de ejemplo"
            >
              <RefreshCw size={18} /> Reestablecer
            </button>
            <button 
              onClick={() => setEditingProduct({ name: '', price: 0, image_url: '', description: '', sales_url: '', order_index: products.length })}
        ...
        >
          {products.map(p => (
            <Reorder.Item 
              key={p.id} 
              value={p}
              className="group flex items-center gap-4 bg-white border border-brand-blue/10 p-4 rounded-2xl cursor-grab active:cursor-grabbing hover:border-brand-blue/30 hover:shadow-md transition-all"
            >
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-6">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-brand-dark/40 ml-1 uppercase">Nombre del Producto (Español)</label>
                  <input 
                    placeholder="Ej. Pack Inicio Keto"
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-brand-dark/40 ml-1 uppercase">Product Name (English)</label>
                  <input 
                    placeholder="Ex. Keto Starter Pack"
                    value={editingProduct.name_en || ''}
                    onChange={e => setEditingProduct({ ...editingProduct, name_en: e.target.value })}
                    className="w-full p-3 rounded-xl border border-brand-blue/10 outline-none focus:ring-2 ring-brand-blue bg-brand-blue/5"
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-brand-dark/40 ml-1 uppercase">Descripción (Español)</label>
                  <textarea 
                    placeholder="..."
                    rows={3}
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-brand-dark/40 ml-1 uppercase">Description (English)</label>
                  <textarea 
                    placeholder="..."
                    rows={3}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-brand-dark/40 ml-1 uppercase">Prioridad de Aparición (Número menor sale primero)</label>
                  <input 
                    type="number"
                    value={editingProduct.order_index || 0}
                    onChange={e => setEditingProduct({ ...editingProduct, order_index: parseInt(e.target.value) || 0 })}
                    className="w-full p-3 rounded-xl border border-brand-blue/10 outline-none focus:ring-2 ring-brand-blue"
                  />
                </div>
